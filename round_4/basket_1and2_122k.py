...
PARAMS = {
    ...
    Product.SPREAD2: {
        ...
        "default_spread_mean": 56.90461666666667,
        # "default_spread_std": 57.91713972894571,
        "default_spread_std": 57.09386067713167,
        "spread_std_window": 75,
...
class Trader:
    ...
    def basket_strategy(self,state):
        orders_croissants = []
        orders_djembes = []
        orders_jams = []
        orders_basket = []
        def create_orders(buy_basket: bool)-> List[List[Order]]:
                ...
                orders_basket.append(
                    Order(Product.BASKET1, price_basket, sign*VOLUME_BASKET)
                )
                orders_croissants.append(
                    Order(Product.CROISSANTS, price_others, -sign*6*VOLUME_BASKET)
                )
                orders_djembes.append(
                    Order(Product.DJEMBES, price_others, -sign*1*VOLUME_BASKET)
                )
                orders_jams.append(
                    Order(Product.JAMS, price_others, -sign*3*VOLUME_BASKET)
                )

        price_basket = self.get_swmid(state.order_depths[Product.BASKET1])
        price_croissants = self.get_swmid(state.order_depths[Product.CROISSANTS])
        price_djembes = self.get_swmid(state.order_depths[Product.DJEMBES])
        price_jams = self.get_swmid(state.order_depths[Product.JAMS])

        position_basket = state.position[Product.BASKET1]  
        spread = price_basket - (6*price_croissants + 1* price_djembes + 3*price_jams)

    # def trade_macarons(self,state):
    #     obs = state.observations.conversionObservations['MAGNIFICENT_MACARONS']
    #     fair_value = self.predict_fair_value({
    #         'sugarPrice': obs.sugarPrice,
    ...
    #     threshold = 2 * historical_std_dev  # Choose based on your backtest

    #     orders = []

    #     # Buy signal
    #     if mid_price < fair_value - threshold and state.position.get('MAGNIFICENT_MACARONS', 0) < 75:
    #         buy_qty = min(75 - state.position.get('MAGNIFICENT_MACARONS', 0), order_depth.sell_orders[best_ask])
    #         orders.append(Order('MAGNIFICENT_MACARONS', best_ask, buy_qty))

    #     # Sell signal
    #     if mid_price > fair_value + threshold and state.position.get('MAGNIFICENT_MACARONS', 0) > -75:
    #         sell_qty = min(state.position.get('MAGNIFICENT_MACARONS', 0) + 75, order_depth.buy_orders[best_bid])
    #         orders.append(Order('MAGNIFICENT_MACARONS', best_bid, -sell_qty))
