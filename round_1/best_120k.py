...
PARAMS = {
    ...
    Product.KELP: {
        "fair_value": 2023,  # Example fair value, could be dynamic
        "take_width": 1,      # How far from fair value to take aggressive orders
        "clear_width": 2,     # How far from fair value to clear inventory aggressively
        # for making
        "disregard_edge": 1,  # Don't penny/join levels this close to fair value
...
class Trader:
    ...
    def kelp_strategy(self, state: TradingState):
        ...
        avg_wt_price = self.compute_avg_prices(product,state)
        ...
        if spread>=2:
            ...
            orders.append(Order(product, math.floor(avg_wt_price) , bid_volume))
            orders.append(Order(product, math.ceil(avg_wt_price), -ask_volume))
    ...
    def squid_ink_strategy(self, state: TradingState):
        ...
        residuals = [p for p in Product.past_prices[product][-50:]]
        recent_prices = Product.past_prices[product][-10:]

        std = np.std(residuals)
        mn = np.mean(residuals)
        slope = recent_prices[-1] - recent_prices[0]
        threshold = std * 3.24
