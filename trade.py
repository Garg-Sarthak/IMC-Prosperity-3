...
class Trader:
    ...
    def run(self, state: TradingState) -> Dict[str, List[Order]]:
        ...
        for product in state.order_depths.keys():
                ...
                # print(f'this is order_depth {order_depth}')
                ...
                acceptable_price = 5600
                if (len(sarthakPrices)) :
                    mn = np.array(sarthakPrices).mean()
                    st = np.std(np.array(sarthakPrices))
                    print(st)
                    acceptable_price = min(mn+2*st,min(mn,mn+st))
                # stdDev = np.array(sarthakPrices).std()
                # print(np.array(sarthakPrices).std())
